...
| Campus stores — `ingest/campus_stores.py` over each institution's own site | `organizations` (store operator + kind in `notes.store`), `contacts` (store mailboxes) | Institution home + store page, robots-checked, 2-4 requests/host | Quarterly re-check | Independent = lead; managed (Follett `efollett.com`, B&N College `bkstr.com`, Akademos, eCampus…) = do-not-chase. Only store-shaped mailboxes are kept — a bursar's address is worse than a blank |
