...
| Campus stores — `ingest/campus_stores.py` over each institution's own site | `organizations` (656 leads, 761 locked), `contacts` (store mailboxes) | Institution home + store page, robots-checked, 2-4 requests/host | Initial pull complete (2026-08-19); monthly workflow rechecks the 400 oldest-checked, full rotation ~12 runs | Independent = lead; managed (Follett `efollett.com`, B&N College `bkstr.com`, Akademos, eCampus…) = do-not-chase. Only store-shaped mailboxes are kept — a bursar's address is worse than a blank |
